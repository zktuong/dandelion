...
# @Last Modified time: 2021-02-01 12:50:07
...
def makedb_igblast(
    fasta, igblast_output=None, germline=None, org="human", extended=True, verbose=False
):
    """
...
def creategermlines(
    ...
    if cloned:
        if mode == "heavy":
            print(
                "            Reconstructing heavy chain {} germline sequences with {} for each clone.".format(
                    germ_type, v_field
                )
            )
            ...
            print(
                "            Reconstructing light chain {} germline sequences with {} for each clone.".format(
                    germ_type, v_field
                )
            )
            if germline is None:
            ...
            print(
                "            Reconstructing {} germline sequences with {} for each clone.".format(
                    germ_type, v_field
                )
            )
            if genotype_fasta is None:
        ...
        if mode == "heavy":
            print(
                "            Reconstructing heavy chain {} germline sequences with {}.".format(
                    germ_type, v_field
                )
            )
            if genotype_fasta is None:
            ...
            print(
                "            Reconstructing light chain {} germline sequences with {}.".format(
                    germ_type, v_field
                )
            )
            if germline is None:
    ...
    if v_germline is None:
        ...
        if os.path.isdir(v_germline):
            ...
            if not os.path.isfile(gml):
                raise OSError(
                    "Input for germline is incorrect. Please rename IGHV germline file to '{}'. Otherwise, please provide path to folder containing the germline IGHV fasta file, or direct path to the germline IGHV fasta file.".format(
                        gml
                    )
                )
        else:
            if not v_germline.endswith(".fasta"):
                raise OSError(
                    "Input for germline is incorrect {}. Please provide path to folder containing the germline IGHV fasta file, or direct path to the germline IGHV fasta file.".format(
                        v_germline
                    )
                )
...
def recipe_scanpy_qc(
    ...
    _adata.obs["percent_mito"] = (
        np.sum(_adata[:, mito_genes].X, axis=1) / np.sum(_adata.X, axis=1) * 100
    )
    ...
    for clus in list(np.unique(_adata.obs["leiden"]))[
        1:
    ...
    _adata.obs["filter_rna"] = (
        (
            pd.Series(
                [min_genes < n > max_genes for n in _adata.obs["n_genes"]],
    ...
    _adata.obs = _adata.obs.drop(
        ["leiden", "leiden_R", "scrublet_cluster_score", "scrublet_score_bh_pval"],
        axis=1,
    )
