...
def clone_rarefaction(self, groupby, clone_key=None, diversity_key=None):
    ...
    pred = pd.DataFrame(
        [np.append(np.arange(1, s, 10), s) for s in res_.sum(axis=1)], index=res_.index
    ).T
...
def clone_diversity(
    ...
    metric=None,
    clone_key=None,
    update_obs_meta=True,
    diversity_key=None,
    resample=False,
    downsample=None,
    n_resample=50,
    normalize=True,
    reconstruct_network=True,
    expanded_only=False,
...
def clone_networkstats(
    self, expanded_only=False, network_clustersize=False, verbose=True
):
    if verbose:
                ...
                if len(clonesizecounts) > 0:
                    if normalize:
                        if len(clonesizecounts) == 1:
                            ...
                            g_c = -np.sum(
                                (clonesizecounts_freqs * np.log(clonesizecounts_freqs))
                                / np.log(len(clonesizecounts_freqs))
                            )
                    else:
                        g_c = shannon(clonesizecounts)
