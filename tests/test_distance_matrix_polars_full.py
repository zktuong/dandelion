...
def test_calculate_distance_matrix_full_multicore(mouse_vdj_membership):
    ...
    from dandelion.utilities._distances import dist_func_long_sep

    # n = df_pandas.shape[0]
    # total_dist = np.zeros((n, n), dtype=float)
    # n = df_polars.height
    # total_distp = np.zeros((n, n), dtype=float)

    # assert np.array_equal(total_dist, total_distp)
    ...
    #     )
    #     seqsp = seq_series.to_numpy()
    #     # Check if we have any non-empty sequences (matching pandas logic)
    #     non_empty_count = sum(1 for s in seqs if s and len(s) > 0)
    #     if non_empty_count <= 1:
    #         print(col, 2)
    #     assert np.array_equal(np.array(seqs), np.array(seqsp))
    #     results1 = pairwise_distances(
    #         seqs,
    #         metric=lambda x, y: (
    #             dist_func_long_sep(
    #                 x,
    ...
    #     total_dist += results1
    #     total_distp += results2
    #     assert np.array_equal(total_dist, total_distp)
    # np.fill_diagonal(total_dist, np.nan)
    # np.fill_diagonal(total_distp, np.nan)
    # assert np.array_equal(total_dist, total_distp, equal_nan=True)
    result_pandas = calc_dist_full_pandas(
